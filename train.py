...
##### 总结 #####
"""
一. 简单介绍一下clip、以及clip的优点是什么？

1、clip是一种用于联合图像和文本的模型, 它能够将自然语言和图像映射到同一个嵌入空间, 从而实现跨模态的理解和应用。clip使用对比学习来训练模型, 目的是
来匹配的图文对之间的相似性最大化, 并且让不匹配的图文对之间的相似性最小化。

2、clip有两个主要部分组成：一个图像编码器和一个文本编码器。图像编码器通常是预训练的视觉模型（resnet系列或者vision transformer系列）。文本编码器
通常是预训练的语言模型（text transformer系列）。
